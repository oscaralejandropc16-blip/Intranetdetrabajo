...
export default function TabLibroIngresos({
  ...
  useEffect(() => {
    ...
    const fetchCorrelatives = async () => {
      try {
        const [corrRes, expRes] = await Promise.all([
          api.get('/rd-intranet/v1/correlatives'),
          api.get('/rd-intranet/v1/expedientes')
        ]);
        if (corrRes.data) {
          setGlobalCorrelatives(corrRes.data);
        }
        if (expRes.data && Array.isArray(expRes.data)) {
          setGlobalExpedientesInfo(expRes.data);
        }
      } catch (error) {
                  ...
                  <td className="px-4 py-3 align-top">
                    {(() => {
                      ...
                      if (isGlobalDuplicate) {
                        ...
                        if (duplicateInfo && duplicateInfo.usuario) {
                          duplicateMessage = `¡Registrado por ${duplicateInfo.usuario}!`;
                        }
                      }
